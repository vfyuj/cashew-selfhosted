...
Future<bool> runAllCloudFunctions(BuildContext context,
  ...
  try {
    ...
    if (e is SelfHostedUnauthenticatedException) {
      ...
      // Per specs/01-local-first-invariant.md this must never block the
      // UI -- just leave sync paused until the user signs in again.
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      ...
      database.watchAllForAutoSync().listen((event) {
        // Must be logged in to perform an automatic sync - selfHostedSession != null
