...
- [x] **Task 1 — dev environment.** Flutter pinned to 3.19.6 (only version compatible with upstream's `intl`/`share_plus` lock combo — newer stable Flutter breaks `pub get` or crashes dart2js on `share_plus` web). Android SDK 34/36 + build-tools + JDK 17 installed via Homebrew, all without sudo. `flutter build apk` and `flutter build web` both verified against pristine `upstream/`.
  - [x] **Resolved 2026-08-11.** `flutter build apk --release` used to fail: `sqlite3_flutter_libs` sets `compileSdk 35`, and AGP 7.3.1's aapt2 cannot parse the Android 35 platform jar (`RES_TABLE_TYPE_TYPE entry offsets overlap actual entry data`) during the release-only `verifyReleaseResources` task. Fixed by upgrading to AGP 8.7.3 / Gradle 8.9 / `compileSdk 35`. The feared knock-on — that a newer toolchain would reintroduce the `pub get`/dart2js breakage that pinned Flutter to 3.19.6 — did not materialise, because Flutter itself was not upgraded, only Gradle and AGP. Full account in `specs/09-releases.md`.
