...
- [x] **Task 1 — dev environment.** Flutter pinned to 3.19.6 (only version compatible with upstream's `intl`/`share_plus` lock combo — newer stable Flutter breaks `pub get` or crashes dart2js on `share_plus` web). Android SDK 34/36 + build-tools + JDK 17 installed via Homebrew, all without sudo. `flutter build apk` and `flutter build web` both verified against pristine `upstream/`.
  - [ ] **Known gap, found 2026-08-10 via CI (GitHub Actions runner, so not a local-environment quirk): `flutter build apk --release` fails.** The `sqlite3_flutter_libs` plugin's own `android/build.gradle` sets `compileSdk 35` independent of the app's `compileSdkVersion 34`, which pulls in the Android 35 platform jar for the release-only `verifyReleaseResources` task. This Flutter 3.19.6 toolchain's aapt2 can't parse that jar (`RES_TABLE_TYPE_TYPE entry offsets overlap actual entry data`) — a real aapt2/SDK-35 incompatibility, reproduced identically on a clean CI runner and a local machine. `flutter build apk --debug` is unaffected (skips that task) and is what `.github/workflows/build-apk.yml` builds today. Likely needs a newer Android Gradle Plugin/build-tools pairing to fix properly, which risks the same `pub get`/dart2js breakage that pinned Flutter to 3.19.6 in the first place — investigate deliberately, not as a side effect of unrelated work.
