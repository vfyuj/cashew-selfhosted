...
**Two Google requests that had nothing to do with Firebase**, found by grepping the built `main.dart.js` for external hosts rather than by reading Dart source — worth repeating as a technique, since neither was visible in the dependency list:
- CanvasKit. Flutter's default `auto` web renderer picks CanvasKit on desktop browsers and fetches `canvaskit.js` from `www.gstatic.com` at launch. Fixed by building with `--dart-define=FLUTTER_WEB_CANVASKIT_URL=/canvaskit/`, pointing at the copy Flutter already writes into `build/web/canvaskit/`.
- CanvasKit's Roboto fallback, from `fonts.gstatic.com`. CanvasKit cannot use system fonts and downloads Roboto during startup for glyph coverage; there is no switch to disable it. Fixed by pinning `--web-renderer html` in the `Dockerfile`, which mobile browsers were already getting under `auto`. **This is a rendering change on desktop web only** — the app's own seven bundled font families are untouched and unaffected. Dropping the flag restores CanvasKit at the cost of one Google request per launch.
