...
- [ ] Acceptance criteria verification: two-device test, airplane-mode test, network inspection for zero Google calls, web-reload-while-signed-in test. Needs a real deployed server or a running/observable app instance.
- [ ] Icon/app-name still placeholder (tracked in Stage 0 status).
...
- No real-time/instant sync (Stage 2).
