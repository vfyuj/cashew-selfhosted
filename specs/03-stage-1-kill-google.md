...
The bundle also still contains a `fonts.gstatic.com` Roboto URL — CanvasKit's glyph fallback. `--web-renderer html` was briefly pinned to avoid it, then **reverted: the owner disliked the resulting rendering, and measurement showed the flag bought nothing.**

~~Loading the CanvasKit build with the network panel open shows CanvasKit served from the app's own origin and **no request to `fonts.gstatic.com`** — the app registers a full `FontManifest.json` of its own seven families, so the engine never needs the fallback. The URL is dead string data on the untaken path.~~ **Wrong, corrected 2026-08-12.** The request was real and had been firing on every CanvasKit launch. The engine's rule has nothing to do with how many families are registered: `canvaskit/fonts.dart` sets `loadedRoboto` only when a family is named *literally* `Roboto`, and downloads `_robotoUrl` otherwise. This fork shipped `RobotoCondensed`, which does not match. Fixed by declaring a `Roboto` family in `app/pubspec.yaml` pointing at the already-bundled `Inter-Regular.ttf` / `Inter-Bold.ttf` — Inter is this app's declared fallback family already, and the entry is only ever consulted for text asking for an unregistered font. Verified with `performance.getEntriesByType('resource')` on a freshly-loaded release build: zero non-same-origin entries except the currency-rates API. This mattered enough to chase because pinning the renderer to `canvaskit` moved every client, phones included, onto the path that made the request.

The lesson worth keeping: that paragraph was written from a plausible mechanism, not from a reading of the engine source, and it then justified itself for months. Re-check runtime Google requests against the network panel on the actual client, and confirm the *mechanism* in `flutter_web_sdk/lib/_engine/` before writing down why something doesn't happen.

**The renderer is now pinned to `canvaskit`, 2026-08-12.** Leaving it at the default `auto` was a mistake that hid for months, because `auto` does not mean "pick the best one" — in the pinned 3.19.6 SDK it is literally `useCanvasKit = isDesktop` (`flutter_web_sdk/lib/_engine/engine/renderer.dart`), and `isDesktop` is false for iOS and Android. So one deploy was shipping **two different rendering engines**: CanvasKit to the desktop browser the owner tested in, and the html renderer — the one rejected on looks in the paragraph above — to every phone. iPhone Safari on the html renderer took a repeatable WebContent crash ("на странице … повторно возникла проблема") on the budgets tab after a real database import. Verified by driving the release build under a mobile user agent: `window.flutterCanvasKit` is `undefined` before the change and set after it, and the budgets tab renders correctly under CanvasKit at a 375×812 viewport. Caveat for whoever revisits this: the crash itself was **not** reproduced locally — generated demo data is far smaller than a real import — so the renderer is the strongly-indicated cause, not a proven one. If it recurs under CanvasKit, the next suspect is raw memory: the web build keeps the whole SQLite file in the JS heap via drift's legacy `WebDatabase` (sql.js) and re-serialises all of it on every write (`app/lib/database/platform/web.dart`), and CanvasKit's own heap use measured ~25% above the html renderer's on the same page.
